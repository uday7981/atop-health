...
function Document({
  ...
  const onBuy = () => {
    if (!write) {
      alert('Error , Press Buy button again');
    }
    write?.();
  };
  ...
  const [enable, setEanable] = useState(false);
  const { config } = usePrepareContractWrite({
    abi: TokenFactoryAbi,
    address: tokenAddress as `0x${string}`,
    functionName: 'replaceAllowedAddress',
    enabled: enable,
    args: [addresses],
  });
  ...
  const { spinner, setSpinner, setSpinnerText } = useContext(SpinnerContext);
  useEffect(() => {
    if (isLoading) {
      if (!spinner) {
        setSpinner(true);
            ...
            >
              ...
              <VStack>
                <Text as='h1' textAlign={'start'}>
                  This will Only show the Files that have a price
                </Text>
              </VStack>
            </Stack>
          </Card>
          ...
          <Card color={'white'} w={'1000px'} marginTop={'10'}>
            ...
            >
              <Stack spacing={1} direction={'row'}>
                ...
                {docs ? (
                  docs.map((doc) => {
                    if (fileToAccessDetail[doc.id - 1]?.price != 0) {
                      return (
                        <Document
...
export default User;
function prepareSendTransaction(arg0: {
  request: { to: string; value: BigNumber };
}) {
